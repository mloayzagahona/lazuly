...
public class ConferenceExtendedJPAImpl extends ConferenceJPAImpl implements ConferenceExtDao {
   ...
   public Conference referConferenceUseCache (Conference conference, boolean isAssign) {
	...
	private String getCacheKey (Conference conferenceWhat, Conference positiveConference, Conference negativeConference, String queryKey) {
	    ...
	    sb.append(queryKey);
	    if (conferenceWhat!=null)
	       sb.append(conferenceWhat.toStringWithParents());
	    if (positiveConference!=null)
	       sb.append(positiveConference.toStringWithParents());
	    if (negativeConference!=null)
	       sb.append(negativeConference.toStringWithParents());
	    return sb.toString();
	}
    ...
    */           
    public boolean existsConference (Conference conference) {
       if (getFirstConference(conference)!=null)
          return true;
       return false;  
    }
        
    public boolean existsConferenceWhereConditionsAre (Conference conference) {
       if (getFirstResultWhereConditionsAre (conference).isEmpty())
          return false;
       return true;  
    }
...
//	 to set in super class
    ...
    */         
	...
	public static String getPartialLoadWithParentWhereConferenceQuery (
	   Conference conference, Boolean isWhereSet, Hashtable aliasHt, Hashtable aliasWhereHt, String childAlias, String childFKAlias) {
	   if (conference==null)
	      return "";
	   String alias = null;
    ...
    public static String getPartialLoadWithParentConferenceQuery (
	   Conference conference, Boolean isWhereSet, Hashtable aliasHt, String childAlias, String childFKAlias, Map beanPath, String rootPath, QueryWhatInit queryWhatInit) {
	   if (conference==null)
	      return "";
	   String alias = null;
	   if (aliasHt == null) {
...
//       query.append(getConferenceSearchEqualQuery (positiveConference, negativeConference));
	...
	protected static String getNextAlias (Hashtable aliasHt, Conference conference) {
		...
		if (cptSameAlias==0)
			...
			return "conference_"+cptSameAlias;
	}
    ...
    public String getPartialLoadConferenceQuery(
	   Conference conference, 
	   Conference positiveConference, 
	   Conference negativeConference) {
       boolean isWhereSet = false;
       StringBuffer query = new StringBuffer();
       if (conference.getId() != null) {
		...
		for (ConferenceFeedback child : children) {
			for (Conference parent : parents) {
				if (parent.getId()!=null && parent.getId().toString().equals(child.getConferenceId()+"")) {
					parent.addConferenceFeedbackConferenceViaConferenceId(child); //ConferenceFeedbackConferenceViaConferenceId
	...
	private void reassembleConferenceMember (List<ConferenceMember> children, List<Conference> parents) {
		for (ConferenceMember child : children) {
			for (Conference parent : parents) {
				if (parent.getId()!=null && parent.getId().toString().equals(child.getConferenceId()+"")) {
					parent.addConferenceMemberConferenceViaConferenceId(child); //ConferenceMemberConferenceViaConferenceId
					child.setConferenceId(parent);
					break;
				}
			}
	...
	private void reassembleSponsor (List<Sponsor> children, List<Conference> parents) {
		for (Sponsor child : children) {
			for (Conference parent : parents) {
				if (parent.getId()!=null && parent.getId().toString().equals(child.getConferenceId()+"")) {
					parent.addSponsorConferenceViaConferenceId(child); //SponsorConferenceViaConferenceId
					child.setConferenceId(parent);
					break;
				}
			}
