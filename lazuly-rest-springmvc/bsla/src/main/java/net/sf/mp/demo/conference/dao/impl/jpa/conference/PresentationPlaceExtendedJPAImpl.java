...
public class PresentationPlaceExtendedJPAImpl extends PresentationPlaceJPAImpl implements PresentationPlaceExtDao {
	...
	public PresentationPlace referPresentationPlace (PresentationPlace presentationPlace, boolean isAssign) {
		...
		if (isAllNull(presentationPlace))
			...
			if (list.isEmpty()) {
			    if (isAssign)
			       insertPresentationPlace(presentationPlace);
			    else
				   return null;
			}
			else if (list.size()==1)
				presentationPlace.copy(list.get(0));
			else 
				//TODO log error
				presentationPlace.copy(list.get(0));
		}
   ...
   public PresentationPlace referPresentationPlaceUseCache (PresentationPlace presentationPlace, boolean isAssign) {
	...
	private String getCacheKey (PresentationPlace presentationPlaceWhat, PresentationPlace positivePresentationPlace, PresentationPlace negativePresentationPlace, String queryKey) {
	    ...
	    sb.append(queryKey);
	    if (presentationPlaceWhat!=null)
	       sb.append(presentationPlaceWhat.toStringWithParents());
	    if (positivePresentationPlace!=null)
	       sb.append(positivePresentationPlace.toStringWithParents());
	    if (negativePresentationPlace!=null)
	       sb.append(negativePresentationPlace.toStringWithParents());
	    return sb.toString();
	}
    ...
    */           
    public boolean existsPresentationPlace (PresentationPlace presentationPlace) {
       if (getFirstPresentationPlace(presentationPlace)!=null)
          return true;
       return false;  
    }
        
    public boolean existsPresentationPlaceWhereConditionsAre (PresentationPlace presentationPlace) {
       if (getFirstResultWhereConditionsAre (presentationPlace).isEmpty())
          return false;
       return true;  
    }
...
//	 to set in super class
    ...
    */         
	...
	public static String getPartialLoadWithParentWherePresentationPlaceQuery (
	   PresentationPlace presentationPlace, Boolean isWhereSet, Hashtable aliasHt, Hashtable aliasWhereHt, String childAlias, String childFKAlias) {
    ...
    public static String getPartialLoadWithParentPresentationPlaceQuery (
	   PresentationPlace presentationPlace, Boolean isWhereSet, Hashtable aliasHt, String childAlias, String childFKAlias, Map beanPath, String rootPath, QueryWhatInit queryWhatInit) {
	   if (presentationPlace==null)
	      return "";
	   String alias = null;
	   if (aliasHt == null) {
...
//       query.append(getPresentationPlaceSearchEqualQuery (positivePresentationPlace, negativePresentationPlace));
	...
	protected static String getNextAlias (Hashtable aliasHt, PresentationPlace presentationPlace) {
		...
		while (keys.hasMoreElements()) {
			String key = keys.nextElement();
			if (key.startsWith("presentationPlace"))
				cptSameAlias++;
		}
		if (cptSameAlias==0)
			return "presentationPlace";
		else
			return "presentationPlace_"+cptSameAlias;
	}
    ...
    public String getPartialLoadPresentationPlaceQuery(
	   PresentationPlace presentationPlace, 
	   PresentationPlace positivePresentationPlace, 
	   PresentationPlace negativePresentationPlace) {
       boolean isWhereSet = false;
       StringBuffer query = new StringBuffer();
       if (presentationPlace.getId() != null) {
		...
		for (Presentation child : children) {
			for (PresentationPlace parent : parents) {
				if (parent.getId()!=null && parent.getId().toString().equals(child.getPresentationPlaceId()+"")) {
					parent.addPresentationPresentationPlaceViaPresentationPlaceId(child); //PresentationPresentationPlaceViaPresentationPlaceId
